...
下面的校验详情可能只描述第一个检测到的问题。返回前必须完整自检：顶层 JSON 形状、全部必填字段和类型、数组数量/顺序/索引、条件 null、所有逐字证据规则和 critique 要求。所有 satisfied=true 的 contract_assessment.goal、must_happen 和 end_state 项，必须先从 source_id=reviewer.full_draft.v1 的【小说草稿】中复制 trim 后非空、连续、1–300 rune 的原文；不得使用契约、场景卡、背景资料或证据候选中的文字，不得概括、改写标点或拼接；找不到正文证据时改为 satisfied=false 并填写未达成原因。contract_assessment.must_happen 和 must_not_happen 必须分别严格匹配输入数量；输入为 0 时输出 []，不得输出 null 或占位项。canon_assessment 有冻结约束时严格匹配约束数量并使用连续 1..N 的 constraint_index；无约束时输出 null。不得交换、合并、新增或补造数组项。
